...
const showSize = () => {
  ...
  const interval = setInterval(() => {
    console.count();
    const result = byteAmount - 5e6;
    byteAmount = result < 0 ? 0 : result;
    updateStatus(byteAmount);
    if (byteAmount === 0) clearInterval(interval);
  }, 50);
};
...
const onload = () => {
  ...
  ioClient.on(ON_UPLOAD_EVENT, (bytesReceived) => {
    ...
    byteAmount = byteAmount - bytesReceived;
